...
public class HIControler : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            sendString.Value = "rotate " + math.sign(frameRotation) * 180 + " " + (math.abs(frameRotation) * speedRotation);
            sendEvent.Raise();
        ...
        else if (math.abs(math.length(frameDirection - lastDirection)) > minDirectionChange && !frameDirection.Equals(float2.zero))
        {
            ...
            {
                sendString.Value = "move " + (math.normalize(frameDirection) * 1000) + " " + (math.length(frameDirection) * speedDirection);
                sendEvent.Raise();
                lastDirection = frameDirection;
            }
            else
            {
                sendString.Value = "move " + (math.normalize(frameDirection) * 1000) + " " + speedDirection;
                sendEvent.Raise();
                lastDirection = frameDirection;
            }
        }
